...
/// This crate has a runtime dependency on artifacts produced by the `ebpf` crate.
...
fn main() {
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `ebpf`, so cargo will automatically rebuild us
        // if `ebpf`'s *library* target or any of its dependencies change. Since we
        // depend on `ebpf`'s *binary* targets, that only gets us half of the way. This
